...
/**
 * Leitor:
 *   ...\public\observer\react\index.php
 *
 * Lido (relativo):
 *   ...\public\frontend_react
 *
 * Modos:
...
// CONFIG
// =========================
$TARGET_REL = __DIR__ . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'frontend_react';
...
{
    if (in_array($name, $ignoreNames, true)) return true;
    if ($isDir) return false;

    $ext = strtolower(pathinfo($name, PATHINFO_EXTENSION));

    if (!empty($ignoreExts) && $ext !== '' && in_array($ext, $ignoreExts, true)) return true;
...
function printTreeType2(string $dir, array $ignoreNames, array $ignoreExts, array $onlyExts, string $prefix = ''): void
{
    ...
    foreach ($items as $name) {
        $full = $dir . DIRECTORY_SEPARATOR . $name;
        $isDir = is_dir($full);
        if (shouldIgnore($name, $isDir, $ignoreNames, $ignoreExts, $onlyExts)) continue;
        $filtered[] = $name;
    }
...
function printType1Numbered(string $baseDir, array $ignoreNames, array $ignoreExts, array $onlyExts): void
{
    ...
    foreach ($rootFiltered as $name) {
        ...
        if (!$isDir) $relPrint .= formatFileSuffix($name);
