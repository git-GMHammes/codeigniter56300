...
use App\Models\v1\ContactUs\FileModel;
use CodeIgniter\HTTP\Files\UploadedFile;

class UploadService
{
    protected string $baseRelativePath = 'uploads/ContactUs'; // caminho relativo usado no DB
    ...
    /**
     * Salva múltiplos arquivos para um contact_us.
     ...
     */
    public function storeMultiple(int $contactUsId, array $files, array $options = [], $db = null): array
    {
        $saved = [];
        $errors = [];
        ...
        foreach ($files as $index => $file) {
            ...
            try {
                // gerar diretorio: writable/uploads/ContactUs/id_123/20251215...
                $dir = $this->baseAbsolutePath . DIRECTORY_SEPARATOR . 'id_' . $contactUsId . DIRECTORY_SEPARATOR . $timestamp;
                if (!is_dir($dir)) {
                    mkdir($dir, 0755, true);
                }
                ...
                $storedPath = $this->baseRelativePath . '/id_' . $contactUsId . '/' . $timestamp . '/' . $filename;
                ...
                if ($db !== null) {
                    ...
                    $db->table('contact_us_files')->insert($insertData);
                } else {
